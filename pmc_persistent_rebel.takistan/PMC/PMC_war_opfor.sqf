...
//if (PMC_debug) then { diag_log format["PMC_war_opfor; PMC_opfor: %1 < _victoryCondition: %2", PMC_opfor, _victoryCondition]; };
		_ran = floor (random 6);
//if (PMC_debug) then { diag_log format["PMC_war_opfor; group create: _ran: %1", _ran]; };
		switch (_ran) do
		{
			case 0:
			{
//if (PMC_debug) then { diag_log "PMC_war_opfor; group create: case 0."; };
...
if (PMC_debug) then { diag_log format["PMC_war_opfor; OPFOR: call PMC_Create_Takistani_Militia! PMC_opfor: %1", PMC_opfor]; };
			...
			case 1:
			{
//if (PMC_debug) then { diag_log "PMC_war_opfor; group create: case 1."; };
...
if (PMC_debug) then { diag_log format["PMC_war_opfor; OPFOR: call PMC_patrol_vehicle_opfor! PMC_opfor: %1", PMC_opfor]; };
			...
			case 2:
			{
//if (PMC_debug) then { diag_log "PMC_war_opfor; group create: case 2."; };
...
if (PMC_debug) then { diag_log format["PMC_war_opfor; OPFOR: call PMC_Create_Takistani_Army_Sniper! PMC_opfor: %1", PMC_opfor]; };
			...
			case 3:
			{
//if (PMC_debug) then { diag_log "PMC_war_opfor; group create: case 3."; };
...
if (PMC_debug) then { diag_log format["PMC_war_opfor; OPFOR: call PMC_Create_Takistani_Army_Support! PMC_opfor: %1", PMC_opfor]; };
			...
			case 4:
			{
//if (PMC_debug) then { diag_log "PMC_war_opfor; group create: case 4."; };
...
if (PMC_debug) then { diag_log format["PMC_war_opfor; OPFOR: call PMC_Create_Takistani_Army! PMC_opfor: %1", PMC_opfor]; };
			...
			case 5:
			{
//if (PMC_debug) then { diag_log "PMC_war_opfor; group create: case 5."; };
//		pmc_var1 = "going to create group: army.";
				...
				publicVariable "PMC_opfor";
//				player sidechat format["opfor case 4, created army. _grp: %1. pmc_opfor: %2", _grp, PMC_opfor];
if (PMC_debug) then { diag_log format["PMC_war_opfor; OPFOR: call PMC_Create_Takistani_Special! PMC_opfor: %1", PMC_opfor]; };
...
//if (PMC_debug) then { diag_log "PMC_war_opfor; groups created, going to choose WP."; };
		...
		_ran = floor (random 3);
//if (PMC_debug) then { diag_log format["PMC_war_opfor; waypoints: _ran: %1", _ran]; };
		switch (_ran) do
		{
			case 0:
		    	{
...
if (PMC_debug) then { diag_log format["PMC_war_opfor; OPFOR: call PMC_Patrol_Inf_Waypoints! _grp: %1", _grp]; };
		    	};
			case 1:
			{
...
if (PMC_debug) then { diag_log format["PMC_war_opfor; OPFOR: call PMC_Guard_Inf_Waypoints! _grp: %1", _grp]; };
			};
			case 2:
			{
...
if (PMC_debug) then { diag_log format["PMC_war_opfor; OPFOR: call PMC_Sentry_Inf_Waypoints! _grp: %1", _grp]; };
			};
		};

		// global once per loop announce ;)
		if (PMC_debug) then { diag_log format["PMC_war_opfor; BLUFOR: %1 (%2), OPFOR: %3 (%4), KIA: %5, FPS: %6", PMC_blufor, PMC_grp_blufor, PMC_opfor, PMC_grp_opfor, PMC_killedNum, diag_fps]; };
	};

//	pmc_var1 = "going to sleep.";
//	publicVariable "pmc_var1";
//if (PMC_debug) then { diag_log format["PMC_war_opfor; going to sleep, time: %1", time]; };
	sleep _sleeptime;
};
...
PMC_war_opfor = false;
publicVariable "PMC_war_opfor";
diag_log format["PMC_war_opfor; EXITED! PMC_opfor: %1. PMC_blufor: %2. time: %3", PMC_opfor, PMC_blufor, time];
