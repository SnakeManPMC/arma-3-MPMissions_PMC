...
if (PMC_debug) then { diag_log format["BASE defense FOR BLUFOR IS DONE: call PMC_Create_US_Army! PMC_blufor: %1", PMC_blufor]; };
...
if (PMC_debug) then { diag_log format["BASE defense IS SET BLUFOR: call PMC_Patrol_Inf_Waypoints.sqf, _grp: %1", _grp]; };
